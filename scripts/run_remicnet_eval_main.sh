...
echo "[run_remicnet_eval_main] output_root=${OUTPUT_ROOT}" | tee "${OUTPUT_ROOT}/logs/run_remicnet_eval_main.log"
PYTHONPATH="${PROJECT_ROOT}" python -m workspace.eval.task_real_008_pipeline \
  --output-root "${OUTPUT_ROOT}" \
  --stage eval | tee -a "${OUTPUT_ROOT}/logs/run_remicnet_eval_main.log"
