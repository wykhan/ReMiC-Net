...
echo "[run_two_stage_training_M2] output_root=${OUTPUT_ROOT}" | tee "${LOG_DIR}/run_two_stage_training_M2.log"
PYTHONPATH="${PROJECT_ROOT}" python -m workspace.train.train_two_stage_et \
  --output-root "${OUTPUT_ROOT}" \
  --mode M2 \
  --epochs 5 \
  --batch-size 4 \
  --smoke-limit 16 | tee -a "${LOG_DIR}/run_two_stage_training_M2.log"
