...
echo "[run_model_audit_800] output_root=${OUTPUT_ROOT}" | tee "${OUTPUT_ROOT}/logs/run_model_audit_800.log"
PYTHONPATH="${PROJECT_ROOT}" python -m workspace.eval.task_real_006d_pipeline \
  --output-root "${OUTPUT_ROOT}" \
  --stage model_audit | tee -a "${OUTPUT_ROOT}/logs/run_model_audit_800.log"
