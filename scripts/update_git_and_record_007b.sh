...
echo "[update_git_and_record_007b] output_root=${OUTPUT_ROOT}" | tee "${OUTPUT_ROOT}/logs/update_git_and_record_007b.log"
PYTHONPATH="${PROJECT_ROOT}" python -m workspace.eval.task_real_007b_geometry_aware \
  --output-root "${OUTPUT_ROOT}" \
  --baseline-root "${BASELINE_ROOT}" \
  --eval006e-root "${EVAL006E_ROOT}" \
  --p1-root "${P1_ROOT}" \
  --stage git | tee -a "${OUTPUT_ROOT}/logs/update_git_and_record_007b.log"
