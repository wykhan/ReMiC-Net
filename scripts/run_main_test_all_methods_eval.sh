...
echo "[run_main_test_all_methods_eval] output_root=${OUTPUT_ROOT} source_root=${SOURCE_ROOT}" | tee "${OUTPUT_ROOT}/logs/run_main_test_all_methods_eval.log"
PYTHONPATH="${PROJECT_ROOT}" python -m workspace.eval.task_real_006e_comprehensive_eval \
  --output-root "${OUTPUT_ROOT}" \
  --source-root "${SOURCE_ROOT}" \
  --stage eval_dataset \
  --dataset-name "Main Test" | tee -a "${OUTPUT_ROOT}/logs/run_main_test_all_methods_eval.log"
