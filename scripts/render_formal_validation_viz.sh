...
echo "[render_formal_validation_viz] output_root=${OUTPUT_ROOT}" | tee "${LOG_DIR}/render_formal_validation_viz.log"
PYTHONPATH="${PROJECT_ROOT}" python -m workspace.eval.formal_scale_validation \
  --output-root "${OUTPUT_ROOT}" | tee -a "${LOG_DIR}/render_formal_validation_viz.log"
