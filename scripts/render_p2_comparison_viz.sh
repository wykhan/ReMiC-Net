...
echo "[render_p2_comparison_viz] output_root=${OUTPUT_ROOT}" | tee "${OUTPUT_ROOT}/logs/render_p2_comparison_viz.log"
PYTHONPATH="${PROJECT_ROOT}" python -m workspace.eval.task_real_007b_geometry_aware \
  --output-root "${OUTPUT_ROOT}" \
  --baseline-root "${BASELINE_ROOT}" \
  --eval006e-root "${EVAL006E_ROOT}" \
  --p1-root "${P1_ROOT}" \
  --stage viz | tee -a "${OUTPUT_ROOT}/logs/render_p2_comparison_viz.log"
PYTHONPATH="${PROJECT_ROOT}" python -m workspace.eval.task_real_007b_geometry_aware \
  --output-root "${OUTPUT_ROOT}" \
  --baseline-root "${BASELINE_ROOT}" \
  --eval006e-root "${EVAL006E_ROOT}" \
  --p1-root "${P1_ROOT}" \
  --stage report | tee -a "${OUTPUT_ROOT}/logs/render_p2_comparison_viz.log"
