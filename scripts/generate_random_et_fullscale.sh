...
echo "[generate_random_et_fullscale] output_root=${OUTPUT_ROOT}" | tee "${LOG_DIR}/generate_random_et_fullscale.log"
PYTHONPATH="${PROJECT_ROOT}" python -m workspace.eval.build_frozen_mainline_handoff \
  --output-root "${OUTPUT_ROOT}" | tee -a "${LOG_DIR}/generate_random_et_fullscale.log"
