...
echo "[run_split_integrity_check] output_root=${OUTPUT_ROOT}" | tee "${LOG_DIR}/run_split_integrity_check.log"
PYTHONPATH="${PROJECT_ROOT}" python -m workspace.eval.formal_scale_validation \
  --output-root "${OUTPUT_ROOT}" | tee -a "${LOG_DIR}/run_split_integrity_check.log"
