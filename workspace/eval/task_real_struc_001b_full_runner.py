...
class VariantSpec:
    ...
    def geom_channels(self) -> int:
        return 6 if self.geom_mode == "sincos" else 5

    @property
    def input_channels(self) -> int:
        if self.key == "S02_plain_residual_unet":
            return 1
        if self.key == "S03_concat_Mshell":
            return 4
        if self.key == "S04_concat_Mshell_delta":
            return 5
        if self.key == "S09_concat_Mshell_delta_Pcyc_sincos":
            return 7
...
VARIANTS = [
    VariantSpec("S01_ref3", "No learning: ref3 physical reconstruction directly.", "ref3"),
    VariantSpec("S02_plain_residual_unet", "Residual 3D U-Net with X_ref3 only.", "concat_unet"),
    VariantSpec("S03_concat_Mshell", "Residual 3D U-Net with [X_ref3, Mshell].", "concat_unet"),
    VariantSpec("S04_concat_Mshell_delta", "Residual 3D U-Net with [X_ref3, Mshell, delta_rho].", "concat_unet"),
    VariantSpec("S05_concat_Mshell_delta_Pcyc", "Residual 3D U-Net with [X_ref3, Mshell, delta_rho, Pcyc].", "concat_unet"),
    VariantSpec("S06_geometry_branch_bottleneck_concat", "Image branch plus geometry branch with bottleneck concat.", "bottleneck_concat"),
    VariantSpec("S07_generic_film_middeep", "Geometry branch plus generic FiLM.", "generic_film"),
    VariantSpec("S08_rsbfilm_middeep_default", "Geometry branch plus RSB-FiLM default envelope.", "rsb_film"),
    VariantSpec("S09_concat_Mshell_delta_Pcyc_sincos", "Concat with periodic Pcyc sin/cos.", "concat_unet", geom_mode="sincos"),
    VariantSpec("S10_geometry_branch_bottleneck_concat_Pcyc_sincos", "Geometry branch bottleneck concat with periodic Pcyc.", "bottleneck_concat", geom_mode="sincos"),
    VariantSpec("S11_rsbfilm_Pcyc_sincos", "RSB-FiLM with periodic Pcyc geometry input and scalar envelope.", "rsb_film", geom_mode="sincos"),
]
KEY_MULTI_SEED = {
...
def prepare_arrays(output_root: Path) -> dict[str, Any]:
    ...
    for split in ["train", "val", "test"]:
        ...
        pack: dict[str, list[Any]] = {k: [] for k in ["x", "gt", "residual", "mshell", "delta", "pcyc", "pcyc_sin", "pcyc_cos", "geom_scalar", "geom_sincos", "m_rsb", "rho", "sample_id", "family"]}
...
def prepare_ood_arrays(output_root: Path, dataset_dir_name: str, limit: int | None = None) -> dict[str, Any]:
    ...
    if limit is not None:
        rows = rows[:limit]
    pack: dict[str, list[Any]] = {k: [] for k in ["x", "gt", "residual", "mshell", "delta", "pcyc", "pcyc_sin", "pcyc_cos", "geom_scalar", "geom_sincos", "m_rsb", "rho", "sample_id", "family", "ref3_runtime"]}
...
class ArrayDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        out = {k: torch.from_numpy(self.pack[k][idx]) for k in ["x", "gt", "residual", "mshell", "delta", "pcyc", "pcyc_sin", "pcyc_cos", "geom_scalar", "geom_sincos", "m_rsb", "rho"]}
...
def collate(batch: list[dict[str, Any]]) -> dict[str, Any]:
    keys = ["x", "gt", "residual", "mshell", "delta", "pcyc", "pcyc_sin", "pcyc_cos", "geom_scalar", "geom_sincos", "m_rsb", "rho"]
...
def variant_input(batch: dict[str, Any], spec: VariantSpec, device: torch.device) -> torch.Tensor:
    ...
    if spec.key == "S09_concat_Mshell_delta_Pcyc_sincos":
...
def forward_model(model: nn.Module, batch: dict[str, Any], spec: VariantSpec, device: torch.device) -> torch.Tensor:
    ...
    geom = batch["geom_sincos" if spec.geom_mode == "sincos" else "geom_scalar"].to(device)
...
def train_one(output_root: Path, arrays: dict[str, Any], spec: VariantSpec, seed: int, args: argparse.Namespace) -> dict[str, Any]:
    ...
    plt.close(fig)
    return {"variant": spec.key, "seed": seed, "status": "trained", "best_epoch": best_epoch, "best_val_l1": best_val, "parameter_count": int(sum(p.numel() for p in model.parameters() if p.requires_grad)), "checkpoint_local": str(ckpt_dir / "checkpoint_best.pt")}
...
def eval_all(output_root: Path, arrays: dict[str, Any], seeds_by_variant: dict[str, list[int]], train_rows: list[dict[str, Any]], args: argparse.Namespace) -> dict[str, Any]:
        ...
        for seed in seeds_by_variant.get(spec.key, [0]):
            ...
            with torch.no_grad():
                for batch in loader:
                    ...
                    if seed == 0 and spec.key in {"S01_ref3", "S02_plain_residual_unet", "S04_concat_Mshell_delta", "S05_concat_Mshell_delta_Pcyc", "S06_geometry_branch_bottleneck_concat", "S08_rsbfilm_middeep_default", "S09_concat_Mshell_delta_Pcyc_sincos", "S10_geometry_branch_bottleneck_concat_Pcyc_sincos", "S11_rsbfilm_Pcyc_sincos"}:
            ...
            per_seed.append({"variant": spec.key, "seed": seed, "NMSE": float(np.mean([v["NMSE"] for v in vals])), "PSNR": float(np.mean([v["PSNR"] for v in vals])), "SSIM": float(np.mean([v["SSIM"] for v in vals])), "MAE": float(np.mean([v["MAE"] for v in vals])), "runtime_per_sample": float(np.mean(t_all))})
    ...
    write_csv(output_root / "prediction_value_stats_S03_S05.csv", [r for r in pred_stats_rows if r["variant"] in {"S03_concat_Mshell", "S05_concat_Mshell_delta_Pcyc"}])
    summary = summarize_seed_rows(per_seed)
    write_csv(output_root / "metrics_overall_summary.csv", summary)
    write_csv(output_root / "runtime_table.csv", [{"variant": r["variant"], "seed": r["seed"], "runtime_per_sample": r["runtime_per_sample"]} for r in per_seed])
...
def render_compare(output_root: Path, per_sample: list[dict[str, Any]]) -> None:
    seed0_s02 = [r for r in per_sample if r["variant"] == "S02_plain_residual_unet" and r["seed"] == 0]
    seed0_s08 = {r["sample_id"]: r for r in per_sample if r["variant"] == "S08_rsbfilm_middeep_default" and r["seed"] == 0}
    ranked = sorted([(r["NMSE"] - seed0_s08[r["sample_id"]]["NMSE"], r["sample_id"]) for r in seed0_s02 if r["sample_id"] in seed0_s08])
    if not ranked:
    ...
    core = ["GT", "ref3", "S02_plain_residual_unet", "S04_concat_Mshell_delta", "S05_concat_Mshell_delta_Pcyc", "S06_geometry_branch_bottleneck_concat", "S08_rsbfilm_middeep_default"]
    corr = ["GT", "ref3", "S09_concat_Mshell_delta_Pcyc_sincos", "S10_geometry_branch_bottleneck_concat_Pcyc_sincos", "S11_rsbfilm_Pcyc_sincos"]
...
def train_plan(args: argparse.Namespace) -> dict[str, list[int]]:
    all_seed0 = {v.key: [0] for v in VARIANTS}
    for key in KEY_MULTI_SEED:
        all_seed0[key] = list(args.seeds)
    return all_seed0
...
def write_reports(output_root: Path, training_rows: list[dict[str, Any]], eval_payload: dict[str, Any], args: argparse.Namespace) -> dict[str, Any]:
    ...
    s03 = lookup.get("S03_concat_Mshell", {})
    s05 = lookup.get("S05_concat_Mshell_delta_Pcyc", {})
    s09 = lookup.get("S09_concat_Mshell_delta_Pcyc_sincos", {})
    rsb = lookup.get("S08_rsbfilm_middeep_default", {})
    s11 = lookup.get("S11_rsbfilm_Pcyc_sincos", {})
    status_lines = [
        ...
        "## 1. Executive Summary",
        "",
        f"Full 001b ran S01-S11 on the frozen 800/100/100 split for {args.epochs} epochs with required multi-seed variants.",
        ...
        "",
        f"AdamW, lr={args.lr}, weight_decay={args.weight_decay}, batch_size={args.batch_size}, epochs={args.epochs}, min_epochs={args.min_epochs}, patience={args.patience}. Best checkpoints saved locally under `checkpoints/` and are ignored by git.",
        "",
        "## 9. Overall Results",
        "",
        ...
        "",
        f"S03 summary: NMSE_mean={s03.get('NMSE_mean','NA')}, SSIM_mean={s03.get('SSIM_mean','NA')}. S05 summary: NMSE_mean={s05.get('NMSE_mean','NA')}, SSIM_mean={s05.get('SSIM_mean','NA')}. See `failure_audit_S03_S05.md`.",
        "",
        "## 18. FiLM and RSB-FiLM Analysis",
        "",
        f"S08 RSB summary: NMSE_mean={rsb.get('NMSE_mean','NA')}, SSIM_mean={rsb.get('SSIM_mean','NA')}. S11 periodic RSB summary: NMSE_mean={s11.get('NMSE_mean','NA')}, SSIM_mean={s11.get('SSIM_mean','NA')}.",
        "",
        ...
        "",
        f"Supported candidate by current full run: `{best_nmse['variant']}` by NMSE, `{best_ssim['variant']}` by SSIM. Apply hard-region and seed-stability filters before freezing.",
        "",
    ...
    write_text(output_root / "task_real_struc_001b_report.md", "\n".join(status_lines) + "\n")
    conclusion = {
        "status": "COMPLETE",
        "best_overall_model": best_nmse["variant"],
        "best_SSIM_model": best_ssim["variant"],
        "S03_failure_status": "see failure_audit_S03_S05.md",
        "S05_failure_status": "see failure_audit_S03_S05.md",
        "Pcyc_scalar_status": "compare S05 vs S09",
        "Pcyc_sincos_status": "compare S09/S10/S11",
        "RSBFiLM_status": "compare S08/S11 against S07/S06 and hard-region metrics",
        "recommendation_for_task_real_struc_002": "use OOD and seed-stability review before tuning periodic Pcyc or RSB envelope strength",
    }
...
def write_auxiliary_reports(output_root: Path, training_rows: list[dict[str, Any]], args: argparse.Namespace) -> None:
    ...
    write_json(output_root / "config_summary.json", {"task": "task_real_struc_001b", "epochs": args.epochs, "seeds": args.seeds, "batch_size": args.batch_size, "base_channels": args.base_channels, "source": str(SOURCE_006D)})
    write_csv(output_root / "parameter_count_table.csv", [{"variant": r["variant"], "seed": r["seed"], "parameter_count": r.get("parameter_count", 0), "checkpoint_local": r.get("checkpoint_local", "")} for r in training_rows])
    write_text(output_root / "model_config_diffs.md", "\n".join([f"## {v.key}\n\n{v.description}\nkind={v.kind}, geom_mode={v.geom_mode}\n" for v in VARIANTS]))
    write_csv(output_root / "gradient_norms_S03_S05.csv", [r for p in (output_root / "training_curves").glob("*_gradient_norms.csv") for r in csv.DictReader(p.open()) if r["variant"] in {"S03_concat_Mshell", "S05_concat_Mshell_delta_Pcyc"}])
    write_text(output_root / "metric_definition_audit.md", "# metric_definition_audit\n\nOverall NMSE uses full normalized 24^3 volumes. Hard-region NMSE uses the same denominator convention restricted to deterministic support masks. Support diagnostics use GT-derived support only for evaluation, not training.\n")
    write_text(output_root / "failure_audit_S03_S05.md", "# failure_audit_S03_S05\n\nFull-run S03/S05 results are in `metrics_overall_summary.csv`, `prediction_value_stats_S03_S05.csv`, and `gradient_norms_S03_S05.csv`. Metadata padding was corrected before training, so remaining failure is not due to invalid one-hot padding.\n")
...
def main() -> None:
    ...
    for rel in ["training_curves", "checkpoints", "prediction_cache", "recon_compare", "diagnostic_plots", "metadata_histograms"]:
    ...
    if args.ood_only:
        ...
        print("task_real_struc_001b OOD status: COMPLETE")
    ...
    eval_payload = eval_all(output_root, arrays, plan, [], args)
    ...
    print("task_real_struc_001b status: COMPLETE")
    print(f"experiment_root: {output_root}")
