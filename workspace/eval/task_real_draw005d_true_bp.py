...
METHOD_SPECS_4X7 = [
    ...
    ("ref3", "ref3", "ref3"),
    ("ref9", "ref9", "ref9"),
    ("pseudo-BP", "pseudo-BP", "pseudo_bp"),
    ("true BP", "true BP", "true_bp"),
...
def render_individual_true_bp(volumes: dict[str, np.ndarray], output_dir: Path) -> None:
    ...
    _plot_volume(ax, volumes["true BP"], "true BP", vmax)
    fig.tight_layout()
    fig.savefig(single_3d / "true_bp_volume.png", dpi=240)
    plt.close(fig)

    fig2, axes = plt.subplots(1, 3, figsize=(9.8, 3.2), squeeze=False)
    _plot_mip(axes[0, 0], volumes["true BP"], "xy", "x-y", colorbar=True)
    _plot_mip(axes[0, 1], volumes["true BP"], "yz", "z-y", colorbar=True)
    _plot_mip(axes[0, 2], volumes["true BP"], "xz", "x-z", colorbar=True)
    fig2.suptitle("true BP", fontsize=12)
    fig2.tight_layout(rect=[0, 0, 1, 0.92])
    fig2.savefig(single_mip / "true_bp_mips_db.png", dpi=240)
    plt.close(fig2)


def render_xz_diagnostic(volumes: dict[str, np.ndarray], rows: list[dict[str, Any]], output_path: Path) -> None:
    methods = ["pseudo-BP", "true BP", "ref3", "ref9", "ref3+U-Net"]
...
def render_tip_comparison(volumes: dict[str, np.ndarray], tip_rows: list[dict[str, Any]], output_path: Path) -> None:
    methods = ["ref3", "ref9", "pseudo-BP", "true BP", "ref3+U-Net"]
...
def write_report(
    ...
    lines = [
        ...
        "## Main figure outputs",
        "",
        f"- 4x7 main figure: `{output_root / 'viz/paper_candidates/manisali_style/dense_y_manisali_4x7_with_true_bp.png'}`",
        f"- 4x7 main PDF: `{output_root / 'viz/paper_candidates/manisali_style/dense_y_manisali_4x7_with_true_bp.pdf'}`",
        f"- 4x5 clean figure: `{output_root / 'viz/paper_candidates/manisali_style/dense_y_manisali_4x5_clean_true_bp.png'}`",
        f"- x-z diagnostic: `{output_root / 'viz/diagnostics/xz_pseudo_bp_vs_true_bp.png'}`",
        "",
        "## x-z bloating analysis",
        "",
        f"At the fixed {XZ_DB_THRESHOLD:.0f} dB x-z projection threshold, pseudo-BP has support area `{pseudo['xz_support_area']}` pixels and bounding box `{pseudo['xz_bbox_width_px']}x{pseudo['xz_bbox_height_px']}`. True BP has support area `{true['xz_support_area']}` pixels and bounding box `{true['xz_bbox_width_px']}x{true['xz_bbox_height_px']}`. In the 3D volume, true BP uses fewer voxels above 0.10 (`{true['support_voxels_ge_0p10']}`) than pseudo-BP (`{pseudo['support_voxels_ge_0p10']}`), but the x-z projection support at this dB threshold is not smaller.",
        "",
...
def run(output_root: Path) -> dict[str, Any]:
    ...
    volumes["pseudo-BP"] = volumes.pop("BP")

    validation = validate_one_voxel(output_root)
    true_bp, bp_meta = run_true_bp(output_root, volumes["GT"])
    volumes["true BP"] = true_bp

    metrics = compute_metrics(volumes)
    tip_input = {**volumes, "BP": volumes["pseudo-BP"]}
    tip_rows, local_rows = analyze_tips(tip_input)
    for row in local_rows:
        if row["method"] == "BP":
            row["method"] = "pseudo-BP"
    for tip in tip_rows:
        nearest_xyz = (int(tip["nearest_voxel_x"]), int(tip["nearest_voxel_y"]), int(tip["nearest_voxel_z"]))
        local_rows.append({"tip": tip["tip"], "method": "true BP", **_local_peak_stats(volumes["true BP"], nearest_xyz)})
    xz_rows = [row for row in metrics if row["method"] in {"ref3", "ref9", "pseudo-BP", "true BP", "ref3+U-Net"}]

    main_path = style_dir / "dense_y_manisali_4x7_with_true_bp.png"
    clean_path = style_dir / "dense_y_manisali_4x5_clean_true_bp.png"
    render_draw005d_composite(volumes, metrics, METHOD_SPECS_4X7, main_path, "Dense-volume Y target: pseudo-BP versus true BP")
    render_draw005d_composite(volumes, metrics, METHOD_SPECS_CLEAN, clean_path, "Dense-volume Y target: clean true-BP comparison")
    render_individual_true_bp(volumes, style_dir)
    render_xz_diagnostic(volumes, metrics, diag_dir / "xz_pseudo_bp_vs_true_bp.png")
    render_tip_comparison(volumes, tip_rows, diag_dir / "tip_local_comparison_true_bp.png")

    write_json(output_root / "metrics_draw005d.json", metrics)
    write_json(output_root / "true_bp_validation.json", validation)
    write_json(output_root / "true_bp_runtime_meta.json", bp_meta)
    write_json(output_root / "xz_bloating_analysis.json", xz_rows)
    write_json(output_root / "tip_analysis_with_true_bp.json", {"tip_rows": tip_rows, "local_rows": local_rows})
    ...
    manifest = {
        ...
        "true_bp_cache": "recon_cache/dense_y_true_bp_display.npz",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate draw005d true-BP dense-Y comparison.")
    parser.add_argument("--output-root", default=None)
    args = parser.parse_args()
    if args.output_root is None:
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_root = Path(__file__).resolve().parents[2] / "exp" / "task_real_draw005d_true_bp" / stamp
    else:
