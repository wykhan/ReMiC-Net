...
def _load_recon(cache_dir: Path, sample_id: str, method: str) -> tuple[np.ndarray, np.ndarray]:
    data = np.load(cache_dir / f"{sample_id}_{method}_accelerated.npz")
...
def render(output_root: Path) -> None:
    ...
    cache_dir = output_root / "accelerated_recon_cache"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Render accelerated point visualization outputs.")
    parser.add_argument("--output-root", required=True)
    args = parser.parse_args()
    render(Path(args.output_root))
    print("Rendered accelerated point visualization outputs")
