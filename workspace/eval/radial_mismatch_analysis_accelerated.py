...
def analyze(output_root: Path) -> dict:
    payload = read_json(output_root / "baseline_metrics_accelerated.json")
    ...
    write_json(output_root / "radial_mismatch_metrics_accelerated.json", result)
    return result
...
def plot_curves(output_root: Path, metrics: dict) -> None:
    ...
    _plot("nmse_vs_rho_target", "nmse_vs_rho_target_accelerated.png", "NMSE")
    _plot("psnr_vs_rho_target", "psnr_vs_rho_target_accelerated.png", "PSNR")
    _plot("ssim_vs_rho_target", "ssim_vs_rho_target_accelerated.png", "SSIM")
    _plot("error_vs_radial_mismatch", "error_vs_radial_mismatch_accelerated.png", "NMSE")

    baseline = read_json(output_root / "baseline_metrics_accelerated.json")["aggregate"]
    methods = METHOD_ORDER
    x = np.arange(len(methods))
    runtime = [baseline[m]["wall_time_mean_sec"] for m in methods]
    speedup = [baseline[m]["speedup_vs_bp"] for m in methods]
    ...
    fig.tight_layout()
    fig.savefig(curves_dir / "runtime_vs_method_accelerated.png", dpi=160)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(curves_dir / "speedup_vs_bp_accelerated.png", dpi=160)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(curves_dir / "quality_vs_method_accelerated.png", dpi=160)
    plt.close(fig)


def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze accelerated radial mismatch curves.")
    parser.add_argument("--output-root", required=True)
    args = parser.parse_args()
    metrics = analyze(Path(args.output_root))
    plot_curves(Path(args.output_root), metrics)
    print("Accelerated radial mismatch analysis complete")
