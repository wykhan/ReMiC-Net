...
def _load_recon(cache_dir: Path, sample_id: str, method: str) -> tuple[np.ndarray, np.ndarray]:
    data = np.load(cache_dir / f"{sample_id}_{method}_variantB.npz")
...
def render(output_root: Path) -> None:
    ...
    cache_dir = output_root / "variantB_recon_cache"
    ...
    for sample_id in representative_ids:
        ...
        for ax, method in zip(axes.ravel(), ["GT"] + METHOD_ORDER):
            if method == "GT":
                ...
                ax.imshow(gt.max(axis=2), cmap="viridis")
                ax.set_title("GT")
            else:
                volume, _ = _load_recon(cache_dir, sample_id, method)
        ...
        for method in ["ref7", "ref9", "BP"]:
            ...
            axes[1].set_title(f"{method} slice")
            axes[2].imshow(np.abs(volume[:, :, z_idx] - gt[:, :, z_idx]), cmap="magma")
            axes[2].set_title("abs error")
            for ax in axes:
                ax.axis("off")
            fig.tight_layout()
            fig.savefig(slice_dir / f"{sample_id}_{method}_slices.png", dpi=160)
            plt.close(fig)


def main() -> None:
    parser = argparse.ArgumentParser(description="Render Variant B confirmation visualizations.")
    parser.add_argument("--output-root", required=True)
    args = parser.parse_args()
    render(Path(args.output_root))
    print("Rendered Variant B confirmation visualizations")
